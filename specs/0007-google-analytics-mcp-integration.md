...
Accepted (2026-08-05) by the SearchCrew maintainer under EVE-33.
...
SearchCrew can read a project's Google Search Console (GSC) property, but an agent
...
The connecting SearchCrew user owns the Better Auth grant. Better Auth stores its
...
Hosted SearchCrew reuses its Google OAuth client. A self-hosted operator reuses
...
The `ga4_connections` row belongs to the SearchCrew project and organization, not
...
custom-metric inputs are excluded from v1. These reports do not consume SearchCrew
credits.
...
SearchCrew does not persist report rows in v1. A later cache needs an approved
...
- Acceptance authorizes implementation work. It does not advertise GA4 as an
  available SearchCrew capability.
...
- [SearchCrew GSC integration decision](./0003-google-search-console-integration.md)
