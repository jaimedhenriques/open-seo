...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew
   ```

2. If the output looks right, apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew --apply --confirm-remote-apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew --validate-only
   ```
...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew --local
   ```

2. Apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew --local --apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database searchcrew --local --validate-only
   ```
