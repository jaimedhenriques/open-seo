...
const BASE = "https://app.searchcrew.ai";
...
type Provider = ReturnType<typeof createSearchCrewOAuthProvider>;
...
beforeEach(async () => {
  ...
  provider = createSearchCrewOAuthProvider(() => new Response("app"));
...
async function callMcp(accessToken: string) {
  ...
  expect(response.status).toBe(200);
  return propsEchoSchema.parse(await response.json()).props.searchCrewAuth;
}
