...
export type SearchCrewOAuthEnv = Env & {
...
function getOAuthHelpers(env: SearchCrewOAuthEnv) {
...
function createDefaultHandler(
  appFetch: AppFetch,
): ExportedHandlerWithFetch<SearchCrewOAuthEnv> {
  return {
...
function createProvider(appFetch: AppFetch, resource: string) {
  const options: OAuthProviderOptions<SearchCrewOAuthEnv> = {
    apiRoute: MCP_ROUTE,
...
export function createSearchCrewOAuthProvider(appFetch: AppFetch) {
  ...
  let provider: OAuthProvider<SearchCrewOAuthEnv> | undefined;
  const getProvider = () =>
  ...
  return {
    ...
    purgeExpiredData(env: SearchCrewOAuthEnv) {
