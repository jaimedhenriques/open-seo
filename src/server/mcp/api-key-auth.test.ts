...
import type { handleAuthenticatedSearchCrewMcpRequest } from "@/server/mcp/transport";
...
const mocks = vi.hoisted(() => ({
  ...
  handleAuthenticatedSearchCrewMcpRequest:
    vi.fn<typeof handleAuthenticatedSearchCrewMcpRequest>(),
}));
...
function request(headers?: HeadersInit, method = "POST") {
  return new Request("https://app.searchcrew.ai/mcp", { method, headers });
}
...
describe("handleMcpApiKeyRequest", () => {
  beforeEach(() => {
    ...
    mocks.handleAuthenticatedSearchCrewMcpRequest.mockResolvedValue(
  ...
  it("leaves non-SearchCrew x-api-key values for OAuth", async () => {
    await expect(
      handleMcpApiKeyRequest(
        request({
