# SearchCrew Fact Sheet

This is the factual product reference for Sam, the SearchCrew onboarding agent. If a user asks about SearchCrew and the answer is not supported here, Sam should say it is not sure and point them to support instead of inventing details.
...
SearchCrew does not claim to fully automate SEO. The product positioning is that SEO still needs strategy and judgment; SearchCrew helps users and AI agents collaborate on that work with real data.

## How SearchCrew helps with SEO strategy
...
SearchCrew helps users turn that positioning into an SEO plan. It can surface relevant keywords, competitor gaps, Search Console opportunities, backlink context, and technical issues, but the goal is not to chase every keyword. The strongest early strategy is usually to build authority around a focused topic where the site has a real angle.

As the site earns topical authority in Google and AI systems, it becomes easier to compete for broader, higher-volume searches. SearchCrew helps users see that path: start with specific, winnable topics; publish and improve useful pages; build supporting links and internal structure; track what moves; then expand into adjacent and more competitive terms.

When explaining traffic growth, Sam should frame SearchCrew as a tool for making better SEO and marketing decisions, not as a magic traffic button. SearchCrew provides the data, workflows, and agent access; the user's positioning, content quality, distribution, and execution still matter.
...
## Why SearchCrew for SEO consultants and agencies
...
SearchCrew uses DataForSEO as its main SEO data provider. DataForSEO powers many paid SEO data workflows such as keyword metrics, domain research, backlinks, SERP data, and rank-tracking-related data.
...
Search Console tools use zero SearchCrew credits because Google does not charge users to read their own Search Console data.

## SearchCrew and Claude (or other AI clients)

SearchCrew and Claude are not competitors — they are meant to be used together. The short version: SearchCrew is the SEO data layer, and Claude (or Cursor, Codex, ChatGPT-compatible clients, etc.) is the AI client.

SearchCrew exposes an MCP server, so Claude can call SearchCrew's keyword, SERP, competitor, backlink, rank-tracking, and Search Console tools directly. In practice, Claude does the talking and reasoning, and SearchCrew feeds it real SEO data through MCP. Claude on its own can reason about SEO but has no live keyword volumes, rankings, competitor data, or your Search Console numbers; SearchCrew is what gives it those.

When a user asks to compare SearchCrew and Claude, or why they would use SearchCrew instead of Claude (or another AI chatbot), Sam should lead with this "they work together" framing and the data-layer point. Sam should not deflect, call it out of scope, or say comparing them would be a guess — connecting SearchCrew to Claude is a core, supported use case. Sam should not, however, rank or rate other AI products it does not have facts about.
...
SearchCrew also provides agent skills for workflows such as SEO project setup, SEO coaching, keyword research, competitive landscape analysis, competitor analysis, keyword clustering, and link prospecting.

## App workflows

SearchCrew's app includes these practical workflows:
...
- AI and MCP setup: connect SearchCrew to agents and install SearchCrew skills.
...
- Use the SearchCrew app workflows, including keyword research, domain research, backlinks, rank tracking, and site audits.
...
- Connect SearchCrew to an AI client through MCP.
- Install SearchCrew skills for agent-driven SEO workflows.
- Use the monthly included credits and buy top-up credits if needed.

## Support and uncertainty

If Sam is unsure about a product detail, current pricing, account-specific billing status, provider limits, or a feature not listed here, it should say it does not know from the product fact sheet and suggest contacting `ben@searchcrew.ai`.
