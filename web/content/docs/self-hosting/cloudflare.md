...
description: "Deploy SearchCrew to your own Cloudflare account for internet-facing, multi-device, or team use."
---
...
Fork `jaimedhenriques/open-seo` on GitHub if you want a repo you control, then clone it locally:
...
Use the same Cloudflare Access application that protects your SearchCrew Worker. Managed OAuth is required for MCP clients and is not enabled by default.

1. Open Cloudflare Zero Trust.
2. Go to `Access controls` -> `Applications`.
3. Find your SearchCrew application, then select `Edit`.
...
## Give teammates access to SearchCrew

Add the teammate to `ACCESS_ALLOWED_EMAILS` in `.env.selfhost` and redeploy. Everyone allowed through shares one SearchCrew workspace.
...
- [Operations](https://github.com/jaimedhenriques/open-seo/blob/main/docs/SELF_HOSTING_CLOUDFLARE_OPERATIONS.md): telemetry and other day-to-day tasks.
- [Legacy deployments](https://github.com/jaimedhenriques/open-seo/blob/main/docs/SELF_HOSTING_CLOUDFLARE_LEGACY.md): maintenance for installs created with the retired Deploy-button or manual Wrangler flows.
